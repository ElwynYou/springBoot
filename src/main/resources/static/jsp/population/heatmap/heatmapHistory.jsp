...
<div class="row page-component-element">
		...
		<form id="searchForm" class="form-inline" action="${ctx}/population/heatmap/heatmapHistory" target="<%=request.getHeader("X-Requested-With") != null?"ajax":"_self" %>">
        ...
        <div class="form-group">
			<div class="input-group">
				...
				<input name="queryTime" value="${heatMapHistory.queryTime}" class="form-control datetime-picker" id="startTime" type="text" > 
			</div>
		</div>
        </div>
        <input hidden="hidden" id="configId" name="configId" value="${heatMapHistory.configId}"/>  
        <input hidden="hidden" id="configName" name="configName" value="${heatMapHistory.configName}"/>  
        <input hidden="hidden" id="queryTimeType" name="queryTimeType" value="${heatMapHistory.queryTimeType}"/> 
      </form>                                   
	</div>
	<div class="row  page-component-element">
    ...
    <div class="col-md-12  col-md-offset-0 tablepad">
            ...
            <thead>
					...
					<tbody>
						<c:forEach items="${heatMapHistoryList}" var="item" varStatus="status" >
							<tr>
							<th>${item.simpStatDate}</th>
							<th><a href="javascript:;" onclick='detailQuery("${heatMapHistory.configId}","${heatMapHistory.configName}","${item.statDate}")'>${item.totalPeople }</a></th>
							<th><a href="javascript:;" onclick='detailQuery("${heatMapHistory.configId}","${heatMapHistory.configName}","${item.statDate}","highRisk")'>${item.attentionPeople }</a></th>
							<th><a href="javascript:;" onclick='detailQuery("${heatMapHistory.configId}","${heatMapHistory.configName}","${item.statDate}","prevent")'>${item.dangerPeople }</a></th>
							<th><a href="javascript:;" onclick='detailQuery("${heatMapHistory.configId}","${heatMapHistory.configName}","${item.statDate}")'>明细</a></th>
							</tr>
						</c:forEach>
					</tbody>         
        </table>
...
$(document).ready(function() {
	...
	function initChart(){
		var AHistory=[];
		<c:forEach items="${heatMapHistoryList}" var="history" >
	...
	var ODetailUrl = {"day":"heatmapDetailByDay","month":"heatmapDetailByMonth","year":"heatmapDetailByYear"};
	function detailQuery(configId,configName,queryTime,crowdType) {
		var url = $.rainsoft.baseUrl() + 'population/heatmap/'+ ODetailUrl[$("#queryTimeType").val()] +'?configId=' + configId +'&configName=' + configName;

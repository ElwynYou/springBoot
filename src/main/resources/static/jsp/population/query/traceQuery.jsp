                ...
                <input type="hidden" name="pageNo" id="pageNo" value="1">
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input name="imsi" id="imsi" type="text" class="form-control" value="${imsiCode}">
                        </div>
                    </div>
                <div class="form-group">
                    <div class="input-group input-medium  input-daterange"
                         data-date-format="yyyy-mm-dd">
                        <span class="input-group-addon">捕获时间</span> <input name="startTime"
                                                                         class="form-control datetime-picker" style="width: 170px" value="${startTime}"
                                                                         id="startTime" type="text"> <span
                            class="input-group-addon">到</span> <input name="endTime"
                                                                      class="form-control datetime-picker" style="width: 170px" value="${endTime}"
...
<script>
    ...
    $(document).ready(function () {
        $.getScript("${mapApi}",function () {//由于百度地图的功能问题,需要延时执行地图加载代码.
            ...
            if($('#imsi').val() && $('#startTime').val() && $('#endTime').val()){
                doTraceQuery();
            }
        });
       ...
       function doTraceQuery() {
            ...
            $.ajax({
                url: $.rainsoft.baseUrl() + "/population/query/list",
                ...
                success: function (data) {
                    ...
                    $('#dataList').next().children().remove();
                    $('#dataList').next().append(data.nav);
                    $('#dataList').next().append("<div style='float: right; line-height: 68px;'>共"+data.page.totalCount+"条记录</div>");
                    var bpoints = [];
                    deletePoint(map);
                    _markerList=[];
                    bpoints=[];
    ...
    function goPage(pageNo) {
        $('#pageNo').val(pageNo);
        $.ajax({
            url: $.rainsoft.baseUrl() + "/population/query/changePage",
            data: {"pageNo": pageNo},
            type: 'post',
            dataType: 'json',
            success: function (data) {
    ...
    function validateSetting(data, messages) {
        return {
            rules: {
